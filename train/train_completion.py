...
train_dataset = SharpNetCompletionDataset(root='C:/Users/sdnyz/PycharmProjects/dataset/shapenetcore_partanno_segmentation_benchmark_v0/',json_path="train_test_split/shuffled_train_file_list.json",w=w, cls_=cls_)
test_dataset = SharpNetCompletionDataset(root='C:/Users/sdnyz/PycharmProjects/dataset/shapenetcore_partanno_segmentation_benchmark_v0/',json_path="train_test_split/shuffled_test_file_list.json", cls_=cls_, w=w)
        ...
        with torch.no_grad():
            for i in range(len(test_dataset)):
                ...
                for j, remain_pc in enumerate(remain_pc_list_):
                    ...
                    #print(shifted.shape)
                    #np.savetxt('../pic/fianal_output_txt' + str(i) + '.txt', shifted, fmt="%f;%f;%f")
                    #
                    crop_list__ = np.array(crop_list_[j])
                    np.savetxt('../pic/fianal_output_txt' + str(i) + '.txt', crop_list__, fmt="%f;%f;%f")
                    #
